...
interface FilterEmailModalProps {
  open: boolean;
  onClose: () => void;
  onApply: (filters: any) => void;
}

export default function FilterEmailModal({ open, onClose, onApply }: FilterEmailModalProps) {
      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="grid grid-cols-3 gap-6 mb-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {['Has Attachment', 'No Attachment', 'All'].map((option) => (
                <label key={option} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="attachment"
                    checked={filters.attachment === option.toLowerCase().replace(' ', '-')}
                    onChange={() => setFilters({ ...filters, attachment: option.toLowerCase().replace(' ', '-') })}
                    className="w-4 h-4 text-purple-600 border-gray-300 focus:ring-purple-500"
                  />
                  <span className="text-xs text-gray-700">{option}</span>
                </label>
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {['Sent', 'Schedule Sent', 'All'].map((option) => (
                <label key={option} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="status"
