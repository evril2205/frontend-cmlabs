...
interface FilterCallModalProps {
  open: boolean;
  onClose: () => void;
  onApply: (filters: any) => void;
}

export default function FilterCallModal({ open, onClose, onApply }: FilterCallModalProps) {
      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="grid grid-cols-3 gap-6 mb-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {['< 5 min', '5-15 min', '> 15 min', 'All'].map((option) => (
                <label key={option} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="duration"
                    checked={filters.duration === option.toLowerCase().replace(' ', '-')}
                    onChange={() => setFilters({ ...filters, duration: option.toLowerCase().replace(' ', '-') })}
                    className="w-4 h-4 text-purple-600 border-gray-300 focus:ring-purple-500"
                  />
                  <span className="text-xs text-gray-700">{option}</span>
                </label>
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {['Completed', 'Missed', 'All'].map((option) => (
                <label key={option} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="status"
