...
interface FilterMeetingModalProps {
  open: boolean;
  onClose: () => void;
  onApply: (filters: any) => void;
}

export default function FilterMeetingModal({ open, onClose, onApply }: FilterMeetingModalProps) {
      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="grid grid-cols-3 gap-6 mb-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {['Online', 'Offline', 'All'].map((option) => (
                <label key={option} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="location"
                    checked={filters.location === option.toLowerCase()}
                    onChange={() => setFilters({ ...filters, location: option.toLowerCase() })}
                    className="w-4 h-4 text-purple-600 border-gray-300 focus:ring-purple-500"
                  />
                  <span className="text-xs text-gray-700">{option}</span>
                </label>
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {['Upcoming', 'Completed', 'Canceled', 'All'].map((option) => (
                <label key={option} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="status"
                    checked={filters.status === option.toLowerCase()}
                    onChange={() => setFilters({ ...filters, status: option.toLowerCase() })}
                    className="w-4 h-4 text-purple-600 border-gray-300 focus:ring-purple-500"
                  />
                  <span className="text-xs text-gray-700">{option}</span>
                </label>
